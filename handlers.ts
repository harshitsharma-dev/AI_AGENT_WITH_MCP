    ...
    private convertStringInputsToLowercase(args: any): any {
        ...
        if (typeof args === 'object') {
            ...
            for (const [key, value] of Object.entries(args)) {
                ...
                const caseSensitiveFields = ['_id', '_key', '_rev', 'pageNumber', 'pageSize', 'limit', 'minWordCount', 
                                           ...
                                           'isCeleb', 'entityIsMovie', 'entityIsTvShow', 'targetEntityIsCeleb', 'coOccurringIsCeleb',
                                           'returnOnlyIsCeleb', 'waitForSync'];
        ...
        return args;
    }// New handler implementations for entity-article analysis    private async handleFindArticlesByEntity(args: FindArticlesByEntityArgs): Promise<any> {
        const { 
            entityName,
            startDate = null,
            endDate = null,
          ...
          const query = `
            FOR entityDoc IN Entity
                FILTER entityDoc.name == @entityName
                LIMIT 1

                FOR v, e IN 1..1 INBOUND entityDoc GRAPH 'newsGraph'
                    FILTER IS_SAME_COLLECTION('Article', v)

                    FILTER @startDate == null OR v.date_added >= @startDate
                    FILTER @endDate == null OR v.date_added <= @endDate
                    FILTER @category == null OR @category IN v.category
                    FILTER @source == null OR v.default.source == @source
                    FILTER @minWordCount == null OR v.word_count >= @minWordCount

                    FILTER @minMentionCount == null OR e.ne_count >= @minMentionCount
                    FILTER @minMentionTF == null OR e.ne_tf >= @minMentionTF
                    FILTER @mentionBertSchema == null OR @mentionBertSchema IN e.bert_schema

                    SORT v.date_added DESC

                    RETURN {
                        articleId: v._id,
                        title: v.default.title,
                        date_added: v.date_added,
                        summary: v.default_summary,
                        category: v.category,
                        source: v.default.source,
                        entityMentionDetails: {
                            count: e.ne_count,
                            tf: e.ne_tf,
                            bertSchema: e.bert_schema,
                            originList: e.origin_list
                        }
                    }
        `;
            ...
            FOR ae IN article_entities
                FOR articleDoc IN Article
                    ...
                    FOR entityDoc IN Entity
                        ...
                        FILTER @isCeleb == null OR entityDoc.isceleb == @isCeleb
                        FILTER @entityIsMovie == null OR entityDoc.ismovie == @entityIsMovie
                        FILTER @entityIsTvShow == null OR entityDoc.istvshow == @entityIsTvShow
                        FILTER @entitySlugContains == null OR CONTAINS(entityDoc.slug, @entitySlugContains)
        ...
        const query = `
            ...
            LET targetEntity = FIRST(
                FOR e IN Entity
                    FILTER LOWER(e.name) == LOWER(@targetEntityName)
                    FILTER @targetEntityIsCeleb == null OR e.isceleb == @targetEntityIsCeleb
            ...
            FOR articleId IN targetEntityArticles
                FOR ae IN article_entities
                    ...
                    FOR e IN Entity
                        FILTER ae._to == e._id
                        FILTER @coOccurringIsCeleb == null OR e.isceleb == @coOccurringIsCeleb
        ...
        const { 
            ...
            LET filterByEntityIds = (
                ...
                FILTER @category == null OR @category IN articleDoc.category
                FILTER @source == null OR articleDoc.default.source == @source
                FILTER @minWordCount == null OR articleDoc.word_count >= @minWordCount
                ...
                LET mentionedEntities = (
                    FOR ae IN article_entities
                        ...
                        FOR entityDoc IN Entity
                            ...
                            FILTER @returnOnlyIsCeleb == null OR entityDoc.isceleb == @returnOnlyIsCeleb
                ...
                RETURN {
                    ...
                    summary: articleDoc.default_summary,
                    category: articleDoc.category,
                    source: articleDoc.default.source,                    topEntities: mentionedEntities
                }
        `;        const cursor = await this.db.query(query, {
            pageNumber,
            pageSize,
